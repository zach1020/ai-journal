...
function createWindow() {
  ...
  mainWindow.webContents.on('did-fail-provisional-load', (event, errorCode, errorDescription, validatedURL) => {
    console.log(`Provisional load failed: ${validatedURL}, Error: ${errorCode} - ${errorDescription}`);
  });
  ...
  const template = [
    {
      ...
      submenu: [
        {
          label: 'About AI Journal',
          click: () => {
            shell.openExternal('https://github.com/zachbohl/ai-journal');
          }
        },
        { type: 'separator' },
        {
    ...
    },
    {
      ...
      submenu: [
        ...
        },
        {
          label: 'Report Issue',
          click: () => {
            shell.openExternal('https://github.com/zachbohl/ai-journal/issues');
          }
        }
      ]
    }
...
// Security: Allow external API calls but prevent new window creation
app.on('web-contents-created', (event, contents) => {
  ...
  // Allow external API calls (like OpenAI)
